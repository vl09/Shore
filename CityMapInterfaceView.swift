...
struct CityMapInterfaceView: View {
    ...
    @State private var landSeaSelection: [Bool] = [false, false, false, false]
    @State private var landSeaStep: Bool = false
            ...
            VStack {
                ...
                if stage < 3 && !landSeaStep {
                    Button(action: {
                        ...
                        Text(stage == 2 ? "Define Land/Sea" : "Next")
                    }
                } else if landSeaStep {
                    VStack {
                        Text("Select Land/Sea Quadrants")
                        Toggle("1st Quadrant", isOn: $landSeaSelection[0])
                        Toggle("2nd Quadrant", isOn: $landSeaSelection[1])
                        Toggle("3rd Quadrant", isOn: $landSeaSelection[2])
                        Toggle("4th Quadrant", isOn: $landSeaSelection[3])
                        Button("Save") {
                            saveAngles()
                            isNavigating = true
                        }
    ...
    private func saveAngles() {
        if cityStorage.cities.contains(where: { $0.id == city.id }) {
            let startAngle = landSeaSelection[0] ? angle1 : (landSeaSelection[1] ? angle2 : (landSeaSelection[2] ? angle1 + 180 : angle2 + 180))
            let endAngle = landSeaSelection[3] ? angle2 : (landSeaSelection[2] ? angle1 : (landSeaSelection[1] ? angle2 + 180 : angle1 + 180))
            ...
            let updatedCity = City(
                ...
                landStartAngle: Double(startAngle),
                landEndAngle: Double(endAngle)
            )
